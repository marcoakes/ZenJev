...
| GitLab issue integration | Keychain `gitlab-token`, account `zenjev` (not created) | **No item exists**; no GitLab account, namespace or token exists for ZenJev | Untested | None. The bounded `--provider=gitlab` read-only smoke has never been run |
...
To add a GitLab credential later, create a distinct Keychain item and never overwrite the Jev entry
(`typesafe-api-key`, account `zenjev`). Supply the value through the prompt, never as a command
argument, and set `GITLAB_CREDENTIAL_REFERENCE` to name the source so the status display can tell a
failed retrieval apart from an absent credential:

```sh
security add-generic-password -s gitlab-token -a zenjev -w
```

The token value must never be printed, committed, placed in a URL or passed in argv. The adapter
sends it only as an `Authorization` header, and its bounded checks redact provider response bodies.
