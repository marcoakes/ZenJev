...
type SmokeResult = { status: 'succeeded'; provider: 'github' | 'zendesk'; readOnly: true; recordsRead: 1; httpRequests: number; modelCalls: 0; mutations: 0; repositoryPrivate?: boolean };
...
export async function integrationSmoke(args: string[], env: Environment, fetcher: typeof fetch = fetch): Promise<SmokeResult> {
  ...
  for (const arg of args) {
    const match = /^(--provider|--repository|--ticket)=(.+)$/.exec(arg);
  ...
  if (provider !== 'github' && provider !== 'zendesk') fail();
  let requests = 0;
  ...
  if (provider === 'github') {
    ...
    if (values['--ticket'] || !repository || repository.length > 200 || !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repository) || repository.split('/').some(segment => segment === '.' || segment === '..') || !allowlist.includes(repository) || !env.GITHUB_TOKEN?.trim()) fail();
  ...
  if (values['--repository'] || !ticket || !/^\d{1,32}$/.test(ticket) || !/^[a-z0-9][a-z0-9-]{0,62}$/.test(subdomain) || !env.ZENDESK_OAUTH_CLIENT_ID?.trim() || !env.ZENDESK_OAUTH_CLIENT_SECRET?.trim() || !scopes || scopes.split(/\s+/).some(scope => !['read', 'tickets:read'].includes(scope))) fail();
