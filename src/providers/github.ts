import {z} from 'zod';
import {type Issue,type ApprovalBinding,type ActionApproval,assertActionAllowed,sanitize} from '../domain';
...
export class GitHubProvider {
  constructor(private readonly options:HttpOptions&{token:string;repositories:string[];apiVersion?:string}){}
  ...
  async listIssues(repository:string,maxPages=20):Promise<Issue[]>{
    const base=this.path(repository),metadata=await this.repository(repository),issues:Issue[]=[];
    for(let page=1;page<=Math.min(maxPages,100);page++){
      ...
      for(const issue of parsed.data.filter(i=>!i.pull_request))issues.push({id:`${repository}#${issue.number}`,repository,number:issue.number,title:issue.title,body:issue.body??'',state:issue.state,labels:issue.labels.map(l=>typeof l==='string'?l:l.name),updatedAt:issue.updated_at,createdAt:issue.created_at,private:metadata.private});
